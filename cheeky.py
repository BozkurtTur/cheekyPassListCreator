...
=====================================|
				    >>>	  :) Good Luck Man
                                     |
			            >>>   Author = Cheeky Family
	  Hacking By Cheeky          |	
				    >>>   Version = 1.0.0
                                     | 
				    >>>	  BozkurtTR ---- Undis --- Herqele
=====================================|
...
def improve_dictionary(file_to_open):
    ...
    conts = input(
        "> Kelime listesindeki tüm kelimeleri birleştirmek ister misiniz? Y/[N]: "
    ).lower()
    ...
    if conts == "y" and len(listic) > CONFIG["global"]["threshold"]:
        ...
        print("[-] Birleştirme için maksimum kelime sayısı\r\n")
        conts = input(
            "> Kelime listesindeki tüm kelimeleri birleştirmek ister misiniz? Y/[N]: "
        ).lower()
    ...
    leetmode = input("> Leet modu? (yani leet = 1337) Y/[N]: ").lower()
    ...
    print("[+] Listeyi sıralama ve kopyaları kaldırılıyor ...")
    ...
    if leetmode == "y":
        for (
            ...
            unique_lista
        ):  # if you want to add more leet chars, you will need to add more lines in cheeky.cfg too...
    ...
    print_to_file(file_to_open + ".cheeky.txt", unique_list_finished)
...
[+] Sözlük yapmak için kurbanla ilgili bilgileri ekleyin """)
    print("""[+] If you don't know all the info, just hit enter when asked!
[+] Bilmediğiniz bilgileri boş bırakabilirsiniz \r\n""")

    # We need some information first!

    profile = {}

    name = input("> ADI: ").lower()
    while len(name) == 0 or name == " " or name == "  " or name == "   ":
        print("\r\n[-] ADINI BİLMEZSEK NASIL WORDLİST OLUŞTURALIM!")
        name = input("> ADI: ").lower()
    profile["name"] = str(name)

    profile["surname"] = input("> SOYADI: ").lower()
    profile["nick"] = input("> TAKMA ADI: ").lower()
    birthdate = input("> DOĞUM TARİHİ (DDMMYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("\r\n[-] GÜN AY YIL OLARAK DEMİŞTİK :)")
        birthdate = input("> DOĞUM TARİHİ (DDMMYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")

    profile["wife"] = input("> EŞİNİN) İSMİ: ").lower()
    profile["wifen"] = input("> EŞİNİN) TAKMA ADI: ").lower()
    wifeb = input("> EŞİNİN) DOĞUM TARİHİ (DDMMYYYY): ")
    ...
    profile["kid"] = input("> ÇOCUĞUNUN İSMİ: ").lower()
    profile["kidn"] = input("> ÇOCUĞUNUN TAKMA İSMİ: ").lower()
    kidb = input("> ÇOCUĞUNUN DOĞUM TARİHİ (DDMMYYYY): ")
    while len(kidb) != 0 and len(kidb) != 8:
        print("\r\n[-] GÜN AY YIL OLARAK GİRMELİSİN :)")
        kidb = input("> ÇOCUĞUNUN DOĞUM TARİHİ (DDMMYYYY): ")
    profile["kidb"] = str(kidb)
    print("\r\n")

    profile["pet"] = input("> HAYVANI VARSA İSMİ: ").lower()
    profile["company"] = input("> İŞ YERİNİN İSMİ: ").lower()
    print("\r\n")

    profile["words"] = [""]
    words1 = input(
        "> Kurban hakkında bazı anahtar kelimeler eklemek ister misiniz? Y/[N]: "
    ).lower()
    words2 = ""
    if words1 == "y":
        words2 = input(
...
# the main function
def main():
    """Command-line interface to the cheeky utility"""

    read_config(os.path.join(os.path.dirname(os.path.realpath(__file__)), "cheeky.cfg"))
